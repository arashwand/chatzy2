...
namespace Messenger.WebApp.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        [RequestSizeLimit(10 * 1024 * 1024)] // 10 MB chunk limit, can be adjusted
...
/*
======================================================================================================================
// SECTION 3: SUGGESTED CODE FOR EXTERNAL WEB SERVICE (CHUNKED FILE UPLOAD)
======================================================================================================================

// =================================================================================
// STEP 1: Add method to IFileManagementService.cs interface
// =================================================================================
/*
...
public interface IFileManagementService
{
    ...
    Task<MessageFile?> ProcessFileChunkAsync(IFormFile file, string uploadId, int chunkIndex, int totalChunks, string originalFileName, long uploaderUserId);
}
*/

// =================================================================================
// STEP 2: Add method implementation to FileManagementService.cs
// =================================================================================
/*
public async Task<MessageFile?> ProcessFileChunkAsync(IFormFile file, string uploadId, int chunkIndex, int totalChunks, string originalFileName, long uploaderUserId)
{
    ...
    var messageFile = await UploadFileAsync(finalFormFile, uploaderUserId, originalFileName);
    ...
    if (messageFile == null || messageFile.Id <= 0)
    {
...
// =================================================================================
// STEP 3: Add action method to FileManagementController.cs
// =================================================================================
/*
[HttpPost("UploadFileChunk")]
[Authorize]
[RequestSizeLimit(10 * 1024 * 1024)] // Same limit as the proxy
public async Task<IActionResult> UploadFileChunk(
    [FromForm] IFormFile file,
    [FromForm] string uploadId,
    [FromForm] int chunkIndex,
    ...
    [FromForm] string originalFileName)
{
    if (file == null || string.IsNullOrEmpty(uploadId) || string.IsNullOrEmpty(originalFileName))
    ...
    try
    {
        ...
        // Call the service method to process the chunk
        var finalFileResult = await _fileManagementService.ProcessFileChunkAsync(file, uploadId, chunkIndex, totalChunks, originalFileName, userId);
        ...
        if (isLastChunk)
        {
            ...
            return Ok(new
            {
                ...
                fileId = finalFileResult.Id // Assuming the entity has an 'Id' property
            });
        }
        else
        {
    ...
    {
        _logger.LogError(ex, "Error processing file chunk for uploadId {UploadId}", uploadId);
